...
SearchSignalsRequest = _reflection.GeneratedProtocolMessageType(
  ...
  Attributes:
      ...
      page_token:
          Page token from an earlier query, as returned in
          ``next_page_token``. All field values except for page_size and
          page_token should be the same as the original query (may
...
SearchIncidentsRequest = _reflection.GeneratedProtocolMessageType(
  ...
  Attributes:
      ...
      query:
          ...
          matches incidents which started after November    11,
          2018.   -  ``start<=2018-11-28`` - matches incidents which
          started on or before    November 11, 2018. -
          ``start:2018-11-28`` - matches incidents which started on
          November    11,     2018.   -  ``start>7d`` - matches
          incidents which started after the point in    time 7*24 hours
          ago -  ``start>180d`` - similar to 7d, but likely to cross the
          daylight    savings time boundary, so the end time will be 1
          hour different from    “now.” -  ``foo AND start>90d AND
...
CancelIncidentRoleHandoverRequest = _reflection.GeneratedProtocolMessageType(
  ...
  Attributes:
      name:
          Required. Resource name of the role assignment.
      new_assignee:
          Required. Person who was proposed as the next assignee (
          i.e. IncidentRoleAssignment.proposed_assignee) and whose
